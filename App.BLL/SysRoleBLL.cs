...
namespace App.BLL
{
    public class SysRoleBLL : BaseBLL, ISysRoleBLL
    {
        ...
        public List<SysRoleModel> GetList(ref GridPager pager, string queryStr)
        {
            ...
            {
                queryData = m_Rep.GetList(dbContainer).Where(a => a.Name.Contains(queryStr));
            }
            else
            {
        ...
        {
            if (dbContainer.SysRoles.SingleOrDefault(a => a.Id == id) != null)
            {
                return true;
            }
            return false;
        ...
        {
            string UserName = "";
            var userList = m_Rep.GetRefSysUser(dbContainer, roleId);
            if (userList != null)
            {
        ...
        {
            IQueryable<P_Sys_GetUserByRoleId_Result> queryData = m_Rep.GetUserByRoleId(dbContainer, roleId);
            pager.totalRows = queryData.Count();
            queryData = m_Rep.GetUserByRoleId(dbContainer, roleId);
